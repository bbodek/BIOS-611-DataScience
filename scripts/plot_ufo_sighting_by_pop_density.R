...
df<-df%>%
  filter(year>=2015)%>%
  group_by(state)%>%
  summarize(sighting_per_100k_peopleyears=100000*sum(sightings)/sum(population),avg_pop_density=round(mean(population)/LandArea))

p<-ggplot(df, aes(x=avg_pop_density, y=sighting_per_100k_peopleyears)) +
  geom_point()+
